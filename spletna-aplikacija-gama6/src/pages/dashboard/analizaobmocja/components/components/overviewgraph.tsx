...
function calculateMovingAverage(data, windowSize) {
  ...
  for (let i = halfWindowSize; i < data.length - halfWindowSize; i++) {
    ...
    const average =
      window.reduce((acc, point) => acc + point.total, 0) / window.length;
    result.push(average);
  }
  return result;
}
  ...
  const data = useMemo(() => {
    ...
    range.forEach((date) => {
      ...
      if (!dailyTotals[formattedDate]) {
        ...
        previousTotal =
          dailyTotals[formattedDate].total / dailyTotals[formattedDate].count;
      }
    });

    let dailyTotalsArray = Object.values(dailyTotals).sort(
  ...
  return (
    <ResponsiveContainer width="100%" height={350}>
      <AreaChart data={data}>
        ...
        />
        <Area type="monotone" dataKey="total" stroke="#4285F460" fill="" />
        <Area
          type="monotone"
