...
export class Scraper {
  ...
  async scrape(
    ...
    const listingData: Listing[] = await page.evaluate((typeOfBuilding) => {
      const listings = Array.from(
        document.querySelectorAll(".col-md-6.col-md-12.position-relative")
      );
      return listings.map((listing) => {
        const title =
          listing
            .querySelector(".font-roboto")
        ...
        const size =
          listing.querySelector(
            'img[src="https://www.nepremicnine.net/images/velikost.svg"]'
          )?.parentElement?.textContent || null;
        const year =
          listing.querySelector(
            'img[src="https://www.nepremicnine.net/images/leto.svg"]'
          )?.parentElement?.textContent || null;
        const location =
          listing
            .querySelector(".url-title-d")
            ?.textContent.replaceAll("\n", "")
            .replaceAll("  ", "") || null;
        const seller =
          listing
            .querySelector(".logo")
            ?.previousElementSibling?.textContent?.replaceAll("\n", "")
            .replaceAll("  ", "") || null;

        const formattedPrice = price
          ? parseInt(
              price.replace("€", "").replace(".", "").split(",")[0].trim()
            )
          : null;
        ...
        const formattedYear = year
          ? parseInt(year.replace("leto", "").trim())
          : null;
